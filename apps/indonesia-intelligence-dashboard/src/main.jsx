...
function App(){
  ...
  return <main>
    ...
    {forex.length > 0 && <section className="grid two marketGrid">
      <Card className="marketCard"><div className="feedHead"><div><div className="sectionTitle"><TrendingUp/> IDR → global currency return radar</div><p>{market.forexBasis}</p></div><div className="filters"><select value={selectedForex?.pair||''} onChange={e=>setForexPair(e.target.value)}>{forex.map(f=><option key={f.pair} value={f.pair}>{f.pair}</option>)}</select></div></div><div className="winner"><span>Most profitable from IDR now</span><strong>{market.bestForex?.pair || selectedForex?.pair}</strong><small>{market.bestForex?.label} · {market.bestForex?.view} · score {market.bestForex?.profitScore}</small></div><ResponsiveContainer width="100%" height={260}><BarChart data={forex}><CartesianGrid strokeDasharray="3 3" stroke="#26324a"/><XAxis dataKey="pair" stroke="#94a3b8" tick={{fontSize:11}}/><YAxis stroke="#94a3b8"/><Tooltip/><Bar dataKey="profitScore" radius={[8,8,0,0]}>{forex.map((d,idx)=><Cell key={d.pair} fill={idx===0?'#22c55e':'#38bdf8'}/>)}</Bar></BarChart></ResponsiveContainer></Card>
      <Card className="marketCard"><div className="sectionTitle"><Activity/> IDR exchanged to {selectedForex?.pair} — return trend</div><p className="marketNote">Last IDR per 1 {selectedForex?.pair}: {money(selectedForex?.last)} · 1M return {pct(selectedForex?.return1m)} · 3M {pct(selectedForex?.return3m)} · 6M {pct(selectedForex?.return6m)} · volatility {pct(selectedForex?.volatility)}</p><ResponsiveContainer width="100%" height={260}><LineChart data={selectedForex?.sparkline||[]}><CartesianGrid strokeDasharray="3 3" stroke="#26324a"/><XAxis dataKey="date" stroke="#94a3b8" tick={{fontSize:10}}/><YAxis stroke="#94a3b8" domain={['dataMin','dataMax']}/><Tooltip/><Line type="monotone" dataKey="value" stroke="#22c55e" strokeWidth={3} dot={false}/></LineChart></ResponsiveContainer><blockquote className="marketQuote">{selectedForex?.useCase}</blockquote></Card>
    </section>}
